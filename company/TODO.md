...
| 7 | **Sign-in security: email code.** Built 2026-09-15: a 6-digit code is emailed at sign-in and must be typed back. It only switches on once a sender is configured: Barbara creates a Gmail app password (Google Account → Security → 2-Step Verification on → App passwords → "hardywu") and gives it to the AI, which puts MAIL_USER / MAIL_PASSWORD in `corporate/.env` and restarts the server. | Barbara: app password · AI: switch on | waiting for the app password |
