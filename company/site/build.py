...
Run from anywhere:  python3 company/site/build.py
"""
import os, re, shutil, sys, tempfile
from pathlib import Path

ROOT = Path(__file__).resolve().parent            # company/site
CORPORATE = ROOT.parent / "corporate"
OUT = ROOT / "www"
os.environ["BOOKKEEP_DATA_DIR"] = tempfile.mkdtemp()      # never touch the real books
os.environ.setdefault("BOOKKEEP_SECRET", "build-only")
sys.path.insert(0, str(CORPORATE)); os.chdir(CORPORATE)
